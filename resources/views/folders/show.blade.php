<x-layout>
    ...
    <h1 class="text-4xl font-bold">{{ $folder->name }}</h1>
    @if (auth()->user()->id != $folder->user_id)
        <h2 class="text-sm text-gray-500 mt-2">by {{ $folder->user->name }}</h2>
    @endif

    @canany(['update', 'delete'], $folder)
        <div class="mt-8 grid grid-cols-2 overflow-hidden border border-gray-300 rounded-md divide-x divide-gray-300">
            <a href="{{ route('folders.edit', ['folder' => $folder]) }}" class="flex items-center justify-center gap-2 hover:bg-gray-950 hover:text-gray-50 py-3 transition">
